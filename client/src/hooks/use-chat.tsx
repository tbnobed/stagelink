...
export function useChat({ sessionId, enabled }: UseChatProps) {
  const { user } = useAuth();
  ...
  }, [enabled, user, sessionId, connect, disconnect]);
  ...
  return {
    ...
    error,
    sendMessage,
    canSendBroadcast: user?.role === 'admin' || user?.role === 'engineer',
  };
}