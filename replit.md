...
- **Settings (merged into Admin)**: Return feed management is now embedded directly in the Admin page. Includes add, edit, delete of studio return feeds (display name, stream name, optional per-feed server address override). The `return_feeds` table is the single source of truth for feed dropdowns on Generator and Productions pages. Default feeds (Socal 1–6, Plex 1–8) are seeded on first setup.
- **WHEP Server Pool**: Admin page includes a dedicated "Return Feed WHEP Servers" section for managing a pool of servers used exclusively for delivering studio return feeds to guests. Servers are assigned round-robin at link generation time. The assigned WHEP server is embedded in the guest link URL as `?returnServer=host:port`. Per-feed server address overrides (in the Return Feeds section) take priority over the pool. Session, viewer, and short-link resolution routes all propagate the assigned WHEP server. The `whep_servers` table stores the pool; `assigned_whep_server` column is added to `generated_links`, `short_links`, `viewer_links`, and `short_viewer_links` tables. `startPlayback()` in `streaming.ts` accepts an optional `serverAddress` parameter to route WHEP connections to the dedicated server.
