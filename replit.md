...
- **Room Backgrounds**: Each room supports a custom background image (`rooms.background_image` column) displayed behind the video grid in both the standard room view and fullscreen output. Images are uploaded via `POST /api/rooms/:id/background` (multipart form, max 10 MB; JPEG/PNG/WebP/GIF), stored in `uploads/room-backgrounds/`, and served statically. The manage page (`/room/:id/manage`) includes a Background Image card for upload, preview, replace, and removal. The preview card on the Rooms page also shows the background.
