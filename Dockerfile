# Virtual Audience Platform v2.5 - Production Docker Build
# Includes: Multi-server WHIP load balancing, consent system, Studio server, rooms
...
COPY . .
# Force clean build - build timestamp: 2025-02-19
...
# Create startup script that ensures complete v2.5 database schema
RUN cat > /app/start.sh << 'EOF'
#!/bin/sh
echo "Starting Virtual Audience Platform v2.5..."
sleep 10

# Ensure database has complete v2.5 schema
...
-- Create generated_links table with assigned_server for multi-server WHIP load balancing
...
-- Create short_links table with assigned_server
...
-- Add assigned_server columns if upgrading from older version
DO $$ BEGIN
...
echo "Virtual Audience Platform v2.5 database schema ready"
echo "Features: User registration, password reset, rooms, consent system, multi-server WHIP load balancing"
echo "US broadcast compliance: CCPA, BIPA, FCC with TBN Adult Likeness Authorization and Release"
